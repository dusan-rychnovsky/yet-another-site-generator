...
mod tests {
  ...
  fn parse_supports_single_file_mode() {
    let args = vec!["yasg".to_string(), "data.yaml".to_string(), "template.html".to_string()];
  ...
  fn parse_single_file_mode_fails_if_too_many_args() {
    let args = vec!["yasg".to_string(), "data1.yaml".to_string(), "data2.yaml".to_string(), "template.html".to_string()];
    let config = Config::parse(&args);
    let err = config.unwrap_err();
    assert_eq!(err, "Single-file mode requires two parameters.");
  }
  ...
  fn parse_supports_recursive_mode() {
    let args = vec!["yasg".to_string(), "-r".to_string(), "src".to_string(), "dst".to_string()];
    let config = Config::parse(&args)
      .unwrap_or_else(|e| panic!("Failed to parse config: {}", e));
    match config.mode {
  ...
  fn parse_recursive_mode_fails_if_too_many_args() {
    let args = vec!["yasg".to_string(), "-r".to_string(), "src".to_string(), "dst".to_string(), "extra".to_string()];
    let config = Config::parse(&args);
    let err = config.unwrap_err();
    assert_eq!(err, "Recursive mode requires two parameters.");
  }
